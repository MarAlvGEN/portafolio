class BlogManager {
	...
	loadData() {
		...
		fetch('posts/data.json')
			...
			.catch(function (err) {
				console.warn(
					'Blog: no posts data found. Run: node scripts/build-blog.js',
				);
				self.renderEmpty();
			});
	}
	...
	getPostTitle(post) {
		...
		if (typeof post.title === 'object')
			return post.title[lang] || post.title['en'] || '';
		return post.title || '';
	}
	...
	getPostDescription(post) {
		...
		if (typeof post.description === 'object')
			return post.description[lang] || post.description['en'] || '';
	...
	renderList() {
		...
		this.posts.forEach(function (post) {
			...
			var imgUrl = post.image
				? 'posts/' +
					encodeURIComponent(post.dir) +
					'/' +
					encodeURIComponent(post.image)
				: null;
			...
			if (imgUrl) {
				card +=
					'<div class="blog-card__image"><img src="' +
					imgUrl +
					'" alt="' +
					title +
					'" loading="lazy" /></div>';
		...
		fetch('posts/' + encodeURIComponent(post.dir) + '/' + mdFile)
			...
			.then(function (content) {
				...
				var postHtml =
					...
					'<span class="blog-post__reading">' +
					self.estimateReading(post.languages[lang].wordCount) +
					'</span>' +
					'</div>' +
					(tags.length > 0
						...
						: '') +
					'</header>' +
					'<div class="blog-post__content markdown-body">' +
					html +
					'</div>' +
					'</div>';
